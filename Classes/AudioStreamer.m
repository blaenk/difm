...
//			}
            ...
            if (lengthNoMetaData > 0)
            {
                ...
                OSStatus err1 = AudioFileStreamParseBytes(audioFileStream, lengthNoMetaData, bytesNoMetaData, kAudioFileStreamParseFlag_Discontinuity);
                if (err1) {
                    //PRINTERROR("AudioFileStreamParseBytes"); myData->failed = true;
                    [self failWithErrorCode:AS_FILE_STREAM_PARSE_BYTES_FAILED];
                    return;
                }
            }
            else if (metaDataInterval == 0)	// make sure this isn't a stream with metadata
            {
                ...
                OSStatus err1 = AudioFileStreamParseBytes(audioFileStream, length, bytes, kAudioFileStreamParseFlag_Discontinuity);
                if (err1) {
                    //PRINTERROR("AudioFileStreamParseBytes"); myData->failed = true;
                    [self failWithErrorCode:AS_FILE_STREAM_PARSE_BYTES_FAILED];
                    return;
...
//			}
            ...
            if (lengthNoMetaData > 0)
            {
                ...
                OSStatus err1 = AudioFileStreamParseBytes(audioFileStream, lengthNoMetaData, bytesNoMetaData, 0);
                if (err1) {
            ...
            else if (metaDataInterval == 0)	// make sure this isn't a stream with metadata
            {
                ...
                OSStatus err1 = AudioFileStreamParseBytes(audioFileStream, length, bytes, 0);
                if (err1) {
                    //PRINTERROR("AudioFileStreamParseBytes");
                    //myData->failed = true;
                    [self failWithErrorCode:AS_FILE_STREAM_PARSE_BYTES_FAILED];
